        ...
        do {
            if(ts->firmware_update_type == 0 || ts->firmware_update_type == 1) {
                if(ts->fw_update_app_support) {
                    ...
                    ret = request_firmware(&fw, fw_name_fae, ts->dev)fw, OPPO_FW_DIR fw_name_fae, ts->dev);
                    if (!ret)
                        break;
                } else {
                    ret = request_firmware(&fw, ts->panel_data.fw_name, ts->dev)fw, OPPO_FW_DIR ts->panel_data.fw_name, ts->dev);
                    if (!ret)
                        break;
                }
            } else {
                ret = request_firmware_select(&fw, ts->panel_data.fw_name, ts->dev)fw, OPPO_FW_DIR ts->panel_data.fw_name, ts->dev);
                if (!ret)
                    break;
            }
        } while((ret < 0) && (--retry > 0));
